...
- RAM: **≥288 GB** for full-scale training and default catalog-scale inference (measured peaks ~260 GB training / ~215 GB inference at the depth-4 default, plus headroom — a strict-256 GB host sits too close to the training peak and risks OOM under page-cache pressure). Means are much lower (~150 GB training / ~36 GB inference); inference RAM scales with `--prefetch-depth` × the largest in-flight cadence, and the depth-4 default's RFI-dense worst case (~5 in-flight × ~65 GB ≈ 325 GB) can exceed the 288 GB floor — hosts at or near that minimum should pass `--prefetch-depth 3`, and smaller-RAM hosts lower it further (run start logs a catalog-derived RAM-preflight warning with a suggested depth when the worst case exceeds ~90% of host RAM — a warning only, nothing is clamped)
...
A **manual** pull (or build) writes no `<sif>.pulled-tag` sidecar, so the wrapper treats the result like a local build: it is never verified or deleted by default — the wrapper warns that it cannot vouch for the image and keeps it across version bumps (set `AETHERSCAN_FORCE_REPULL=1` to delete it and re-pull the published image instead, or `rm` the `.sif` yourself when you bump versions). Let `run_container.sh` do the pulling if you want provenance tracked for you: the sidecar records the pinned ref (`repo:tag`, so both a version bump and an `AETHERSCAN_IMAGE` change trigger a re-pull) plus the image's manifest digest, which is re-checked against the registry on every run — if the remote digest has moved (a retag), the wrapper warns, deletes, and re-pulls automatically ([#424](https://github.com/zachtheyek/Aetherscan/issues/424)).
...
A local build placed over the default `.sif` path is safe: `run_container.sh` caches pulled images with a `<sif>.pulled-tag` sidecar and detects a locally-built `.sif` by mtime, so your build is kept and never overwritten by a pull — even across version bumps. The wrapper does warn that it cannot vouch for an image it didn't pull; only setting `AETHERSCAN_FORCE_REPULL=1` deletes it and replaces it with the published image ([#424](https://github.com/zachtheyek/Aetherscan/issues/424)).
