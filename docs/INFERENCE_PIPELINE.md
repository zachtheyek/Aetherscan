...
~30 h of training — measured on the 20-round pre-#372 default; a 50-round run is
...
4. **Prefetch depth = `inference.prefetch_depth`** (default 4 — the #406 grid searches
   ...
   ROW ORDER, log interleaving, and the latent-projection figure's bounded non-candidate
   subsample (its viz-only budget fills at collection time in completion order; candidates
   are always kept, and every other figure is rendered from catalog-sorted records) — row
...
deliberately NON-reproducible remainder: manifest/DB row insertion order, log-line
interleaving, and the latent-projection figure's bounded non-candidate subsample (a
viz-only collection-time budget; candidates always kept, all other figures render from
catalog-sorted records) follow completion order and vary run-to-run (science content is
order-free by construction — nothing in resume, queries, or reporting keys on row order).
...
| `candidate_frequency_map_{tag}.png` | A dot-histogram (#423): frequencies binned along x, each bin's candidates **stacked bottom-up as dots** so column height reads as candidates-per-bin (log-scale y past 10⁴ total candidates, or past a 400-deep single bin — the trigger that fires on coincidence-heavy runs), **colored by target** with legend labels (top-18 targets by candidate count, distinct hues first + one gray aggregate entry; band is deliberately absent from the legend — x-position implies it), report-excluded ranges (#395) shaded. | The load-bearing read is one TALL, MULTI-COLOR column (#394): the same frequency stacking high across many targets is the multi-target-coincidence signature of a terrestrial transmitter; a genuine technosignature is a short single-color stack. [The RFI-triage report](#the-candidate-rfi-triage-report-utilscandidate_rfi_reportpy) (#396) quantifies the same signal off-cluster. |
...
A standalone, **read-only** review tool over one run's live candidates (#396) — the
off-figure counterpart of `candidate_frequency_map_{tag}.png`. It opens the run DB with a
`mode=ro` SQLite URI, reads the tag's live `inference_results` rows (`prediction=1`,
superseded excluded), and uses only stdlib `sqlite3`/`csv`/`argparse` plus numpy — no
`aetherscan` imports — so it also runs off-cluster against a DB pulled with
`utils/fetch_run_outputs.sh --db`. It reports three RFI signals: **multi-target frequency
coincidence** (binned; `--coincidence-bin-mhz`, `--min-targets`), **known-allocation flags**
(GPS/GLONASS/Iridium bands), and **per-target concentration**. `--exclude-frequency-range
START_MHZ END_MHZ` (repeatable) applies the same report-time exclusion accounting as the
pipeline's `--report-exclude-frequency-range` (#395). It **flags only, never deletes** —
the DB is not writable through it by construction.

```bash
python utils/candidate_rfi_report.py --save-tag <tag>            # on-cluster, default DB
python utils/candidate_rfi_report.py --save-tag <tag> \
    --db-path outputs/data/db/aetherscan.db --csv flagged.csv    # off-cluster, fetched DB
```
