...
| [`candidate_triage.py`](../src/aetherscan/candidate_triage.py)                                                             | Report-time candidate triage (#404): frequency-exclusion partitioning for tallies/report surfaces and Mahalanobis OOD scores for review ordering. Strictly report-time — it never changes what a snippet scores, what lands in `inference_results`, or what is rendered — which is why `report_exclude_frequency_ranges` sits in `run_state.py`'s fingerprint denylist. See [`INFERENCE_PIPELINE.md`](INFERENCE_PIPELINE.md). |
...
| [`display_tag.py`](../src/aetherscan/display_tag.py)                                                                       | Derives the presentation-only `{command}_{machine}_{datetime}` display tag used for artifact filenames, plot titles and Slack messages (stdlib-only, import-cycle-safe); nothing that keys a DB row — `save_tag` included — changes. See [Tag conventions](#tag-conventions). |
