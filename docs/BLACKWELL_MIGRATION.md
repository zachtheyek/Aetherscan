...
```bash
./scripts/run-container.sh python -c "
...
You're running TF on the host directly, not via the container. The host TF wheel has no sm_120 kernels. Re-run through `./scripts/run-container.sh`.
...
NCCL_DEBUG=INFO ./scripts/run-container.sh python -m aetherscan.main train ...

# Or force the hierarchical fallback by lowering num_packs
./scripts/run-container.sh python -m aetherscan.main train --nccl-num-packs 1 ...
```
...
```bash
./scripts/run-container.sh python -m aetherscan.main train --no-async-allocator ...
```
