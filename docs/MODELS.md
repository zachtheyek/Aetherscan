...
| `random_forest_{tag}.joblib` | `train_random_forest` / `save_models` | The fitted sklearn classifier (joblib of the `RandomForestClassifier`) — the *winning* variant of the #282 sweep, under the canonical name. Carries `aetherscan_latent_variant_` + `aetherscan_active_dims_` attributes stamping the representation it was trained on, so inference can verify the config's declared `rf.latent_variant` / `rf.active_dims` against the forest itself — the feature-count guard alone cannot separate the same-width variants (`z`/`z_mean`/`z_aug`) or two equal-length active-dim sets for `z_mean_logvar_active` (#318). Forests trained before #318 (incl. v1.0.0) lack the stamps; inference no-ops those identity checks for them. |
| `random_forest_{tag}_{variant}.joblib` (×8) | `train_random_forest` | Every variant's fitted forest from the sweep, kept for post-hoc comparison; only the winner is deployed. Since #318 each also carries the `aetherscan_latent_variant_` / `aetherscan_active_dims_` self-description stamps (they are the likeliest source of a same-dir, same-tag mispairing the inference guard exists to catch). |
...
| `rf_shap_values_{tag}.joblib` | `_compute_or_load_shap_values` | Cached SHAP outputs: positive-class summary values (+ the val row indices they correspond to), pairwise interaction values, a log-loss decomposition (`model_output="log_loss"`) normalized across shap versions by `_select_positive_class_shap`, and — since #414 — an `input_fingerprint` (sha256 of the train + eval matrices + labels + the persisted RF artifact's bytes) verified on reuse, so a cache left by an older model/val split recomputes instead of being served. Computing these is minutes of work; every SHAP figure reuses the cache. |
| `rf_shap_clustering_{tag}.joblib` | `plot_rf_shap_explanation_clustering` | Cached `{embedding, cluster_labels, shap_fingerprint}` for the SHAP-space clustering figure: a seeded 2D UMAP embedding of the summary SHAP matrix (`n_summary × F`, the winning variant's feature width — 48 for the default variants, up to 96) plus a KMeans (`k=4`) label per row computed over that SHAP space (not the 2D embedding), plus a content hash of the exact SHAP matrix the fit consumed. Kept separate from the raw-latent `umap_*` reducers below (fit on latents, not SHAP), so re-rendering this plot reproduces the identical layout without a re-fit. Since #359 both this cache and `rf_shap_values_{tag}.joblib` self-validate on load (shape/config checks + the #414 input fingerprint on the SHAP cache; the content fingerprint here) and recompute/refit automatically when stale — no manual deletion needed after changing `shap_max_samples_summary`; pre-#359/pre-#414 caches without their fingerprint recompute/refit once, and the pair can no longer be coherently stale against a newer model (#414). |
