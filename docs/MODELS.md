...
| `random_forest_{tag}.joblib` | `train_random_forest` / `save_models` | The fitted sklearn classifier (joblib of the `RandomForestClassifier`) — the *winning* variant of the #282 sweep, under the canonical name. Carries an `aetherscan_latent_variant_` attribute stamping which variant it was trained on, so inference can verify the config's declared `rf.latent_variant` against the forest itself — the feature-count guard alone cannot separate the same-width variants (`z`/`z_mean`/`z_aug`) (#318). Forests trained before #318 (incl. v1.0.0) lack the stamp; inference no-ops the identity check for them. |
