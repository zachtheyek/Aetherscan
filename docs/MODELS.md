...
| `rf_shap_values_{tag}.joblib` | `_compute_or_load_shap_values` | Cached SHAP outputs: positive-class summary values (+ the val row indices they correspond to), pairwise interaction values, a log-loss decomposition (`model_output="log_loss"`) normalized across shap versions by `_select_positive_class_shap`, and — since #414 — an `input_fingerprint` (sha256 of the eval matrix + labels + the persisted RF artifact's bytes) verified on reuse, so a cache left by an older model/val split recomputes instead of being served. Computing these is minutes of work; every SHAP figure reuses the cache. |
