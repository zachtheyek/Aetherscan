...
| `rf_shap_clustering_{tag}.joblib` | `plot_rf_shap_explanation_clustering` | Cached `{embedding, cluster_labels}` for the SHAP-space clustering figure: a seeded 2D UMAP embedding of the summary SHAP matrix (`n_summary × F`, the winning variant's feature width — 48 for the default variants, up to 96) plus a KMeans (`k=4`) label per row computed over that SHAP space (not the 2D embedding). Kept separate from the raw-latent `umap_*` reducers below (fit on latents, not SHAP), so re-rendering this plot reproduces the identical layout without a re-fit. Delete it **together with `rf_shap_values_{tag}.joblib`** after changing `shap_max_samples_summary` — that SHAP cache is keyed on `{tag}` alone too, so deleting the clustering cache alone would just re-fit over the same stale SHAP matrix. |
