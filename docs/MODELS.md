...
| `rf_shap_clustering_{tag}.joblib` | `plot_rf_shap_explanation_clustering` | Cached `{embedding, cluster_labels}` for the SHAP-space clustering figure: a seeded 2D UMAP embedding of the summary SHAP matrix (`n_summary × 48`) plus a KMeans (`k=4`) label per row over that same SHAP space. Kept separate from the raw-latent `umap_*` reducers below (those are fit on latents, not SHAP), so re-rendering just this plot reproduces the identical cluster layout without a UMAP/KMeans re-fit. |
