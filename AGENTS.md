...
- Every PR links an existing issue (`Closes #N`); branch prefixes `feature/`/`hotfix/`/`misc/`/`Codex/`; rebase (not merge) onto `master`; commits need **verified GPG signatures**; all pre-commit hooks must pass (ruff-format may rewrite files → `git add` again before re-committing).
- **Don't tag the assistant unintentionally.** The assistant handle (an `@` immediately followed by `Codex`) in a Discussion/issue/PR title or body triggers the assistant workflow (`Codex.yml`) — write it only when you actually want to invoke the assistant. To reference it as plain text, write `"@ Codex"` (space after the `@`, double quotes on both sides) so the trigger can't match.
...
Every PR gets an automated `Codex-review` first pass. **Wait for it to land**, then work each note individually — weigh it against your own read of the code, don't take it on faith.
