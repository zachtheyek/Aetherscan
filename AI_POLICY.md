...
Aetherscan has strict rules for AI usage:
...
- **Pull requests created in any way by AI can only be for accepted issues.** "Drive-by" pull requests that do not reference an accepted issue will be closed without explanation. If AI isn't disclosed but a maintainer suspects its use, the PR will also be closed without explanation. If you want to share code for a non-accepted issue, open a discussion or attach it to an existing discussion.
