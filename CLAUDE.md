...
# release-pinned GHCR image on first use when no local .sif exists
...
- Releases are SemVer (`vX.Y.Z`): the compatibility contract covers the CLI, artifact/config formats + model contract, and supported runtimes; highest bump wins, and **four** objects share one version string — git tag/GitHub Release + PyPI, HF weights tag, GHCR runtime image (a same-contract retrain is at least a MINOR). Full policy: [docs/RELEASE.md](docs/RELEASE.md#versioning-policy-semver).
