...
- Releases are SemVer (`vX.Y.Z`): the compatibility contract covers the CLI, artifact/config formats + model contract, and supported runtimes; highest bump wins, and code + weights share one version string (a same-contract retrain is a MINOR). Full policy: [docs/RELEASE.md](docs/RELEASE.md#versioning-policy-semver).
